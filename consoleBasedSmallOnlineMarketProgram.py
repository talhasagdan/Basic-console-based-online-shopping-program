...
def main_menu():  #the function for user to make decision about what they want to do
    mainmenu=input("Please choose one of the following services:" +"\n\t"+"1. Search for a product" +"\n\t"+"2. See Basket"+"\n\t"+"3. Check Out"+"\n\t"+"4. Logout"+"\n\t"+"5. Exit"+2*"\n"+"Your Choice:")

    if mainmenu=="1":
        searchingfor = input("What are you searching for?\n").lower()
        if searchingfor==0:
            main_menu()
        else:
            search(searchingfor)
...
def wrongchoice(searchingfor):      #the wrongchoice function created for if users input does not match any items.
    while True:
        if len(list)==0:
            wrongchoice = input("Your search did not match any items. Please try something else (Enter 0 for main menu):").lower()
...
def basketma(searchingfor):     #the basketma function is for saving the choosen items in the users basket
    select = input("Please select which item you want to add to your basket (Enter 0 for main menu):" )
    i = 0
    if select=="0":
        return main_menu()
    else:
...
def amountfunc(items):  #the function for determine the amount
    amount = input("Adding " + items + ". Enter Amount:")
...
def updateamount():     #The updadete amount function for user to be able to make changes in the amounts of the products that they added into their baskets
    changeamount=input("Please select which item to change its amount:")
    i=0
...
def updateamount_2(items):  #the updateamout_2 function is for user to write the new amount they want
    changeamountnumber = input("Please type the new amount:")
...
def delitem():    # the function for deleting items from the inventory
    remove=input("Please select which item to remove from your basket:")
    i = 0
    for items in basket[username]:
        i = i + 1
        if str(i) == remove:
